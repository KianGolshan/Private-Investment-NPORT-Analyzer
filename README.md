...
Fund X-Ray's **Top Funds** dropdown is powered by `data/fund-index.json`, a
pre-built shortlist generated by `npm run build-fund-index`
(`scripts/build-fund-index.js`). It's an easy-access convenience list, not a
scientific ranking, assembled from two discovery sources that funnel into
one pool of candidate funds:

1. **`fund-candidates.js`** — fund/registrant names from well-known asset
   managers publicly reported to carry meaningful private-equity marks
   (American Funds/Capital Group, Fidelity, BlackRock, T. Rowe Price,
   Franklin, Dimensional, Baillie Gifford, Morgan Stanley Institutional,
   Janus Henderson, Baron Capital, ARK, and others), resolved to a CIK via
   the same EDGAR company-name lookup the manual Fund X-Ray search uses.
   This only works when a candidate string happens to match the
   registrant's own legal name on EDGAR (a "starts with" match, not the
   fund's marketing name), so it misses funds organized under a
   differently-named multi-series trust.
2. **`private-company-seeds.js`** — well-known private companies (OpenAI,
   Anthropic, SpaceX, Stripe, Databricks, Ramp, Discord, Canva, Plaid, xAI,
   Scale AI, ByteDance, Anduril, and more). Each name is run through the
   same EDGAR full-text search Single Security search already uses,
   restricted to NPORT-P filings, and every fund whose *own filing* reports
   a holding by that name is pulled in directly by CIK — no guessing
   involved, and it catches funds source #1 misses.

For every unique CIK found by either source, the script pulls its NPORT-P
filing history (trailing 3 years only), scans a handful of its most recent
filings (a single registrant CIK is often a multi-series trust filing many
unrelated funds, so the single latest filing could be the wrong sibling),
and keeps the fund only if its best-scanned filing shows at least $1M of
real private-equity exposure — just enough to exclude a stray few-thousand-
dollar rounding/legacy position, not a "big fund only" filter. Survivors are
sorted by $ exposure for display convenience and capped at 150. As of the
last run this surfaced 111 real, EDGAR-verified funds.

Rerun `npm run build-fund-index` periodically (each new NPORT-P filing
quarter) to keep the list current, or after adding names to either seed
file. Each parsed filing is cached indefinitely (same cache the live app
uses), but each fund's filing-history lookup is re-fetched from EDGAR fresh
every run, so a full rerun still takes a few minutes even when nothing has
changed.
...
├── fund-candidates.js # Seed fund names for the Fund Index build script
├── private-company-seeds.js # Seed private companies for Fund Index discovery
├── scripts/
│   └── build-fund-index.js # Builds data/fund-index.json (Fund X-Ray "Top Funds" list)
├── data/
│   └── fund-index.json # Generated: pre-built Top Funds shortlist (see Fund Index)
├── public/
...
| `/api/fund-index` | GET | Serves the pre-built Top Funds shortlist (`data/fund-index.json`); 404 with a build hint if it hasn't been generated yet |
